...
The exact-payload review is continued in [packaging/REDISTRIBUTION_REVIEW.md](packaging/REDISTRIBUTION_REVIEW.md). The native Touchstone utility adds no runtime family: its audited closure uses the same three VC DLLs. MUMPS CeCILL-C and Eigen MPL covered-source access are addressed by including corresponding source in the planned payload, not by treating download availability as permission. Publisher Visual Studio Community eligibility is now owner-confirmed. Embedded Intel compiler-support license scope and the remaining distribution conditions remain unresolved; the candidate is not declared redistributable.

Exact linked-code review additionally found LGPL-2.1-or-later GKlib sorting macros inside the pinned METIS implementation. The top-level Apache license is not the complete license picture. Linked `metis:gklib.c.obj` routines and the outstanding static-combination/relinking obligations are documented in the exact-payload review; no redistribution pass is claimed.

Current review: [the single exact matrix](packaging/REDISTRIBUTION_MATRIX.md) supersedes the historical component-status tables for the working replacement closure. Exact Intel member coverage and tested shared-METIS replacement are now recorded; the remaining concrete licensing question is LGPL section 6 modification/debugging permission versus Intel compiler/oneMKL restrictions. The original ZIP is unchanged and remains uncleared. See the final section of [the detailed review](packaging/REDISTRIBUTION_REVIEW.md); no new package or Gate 0 pass is declared.
