...
The exact-payload review is continued in [packaging/REDISTRIBUTION_REVIEW.md](packaging/REDISTRIBUTION_REVIEW.md). The native Touchstone utility adds no runtime family: its audited closure uses the same three VC DLLs. MUMPS CeCILL-C and Eigen MPL covered-source access are addressed by including corresponding source in the planned payload, not by treating download availability as permission. Publisher VS entitlement and embedded Intel compiler-support license scope remain explicitly unresolved; the candidate is not declared redistributable.
