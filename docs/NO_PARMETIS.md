...
### Complete native candidate

The subsequent native build produced `palace.exe` and completed Electrostatic, Magnetostatic, two-excitation Driven and two-mode ARPACK Eigenmode cases. The final closure audit includes installed archives (vendor symbol prefixes and CRT directives), exported CMake targets, the executable link map and recursive PE imports. No ParMETIS DLL, static library or vendor implementation object was found. Original source pins and patch hashes remain in the lock; MUMPS's disabled-feature wrappers and Palace's preserved enum strings are not ParMETIS implementation code.

An actual Palace configuration with `ColumnOrdering: ParMETIS` returns a nonzero exit and the explicit unsupported-ordering diagnostic. The corresponding METIS case runs in the staged Unicode/space path. Upstream memory-reporting and CSV tests passed 80 assertions in 11 cases. Four solver output checks establish finite CSV/VTU data, complete 2x2 S columns, referenced VTK files and input-tolerance eigen residuals; Linux comparison and independent viewer checks remain separate.
